...
class OrdenesDeReparacion(models.Model):
    _inherit = 'ordenes.de.reparacion'

    def recalculate_prices(self):
        for line in self.mapped('refacciones_ids'):
            dict = line._convert_to_write(line.read()[0])
            if 'product_id' in line._fields:
                dict['product_id'] = line.product_id
            line2 = self.env['refacciones.ordenes.de.reparacion'].new(dict)
    ...
    def recalculate_names(self):
        for line in self.mapped('refacciones_ids').filtered('product_id'):
            # we make this to isolate changed values:
            line2 = self.env['refacciones.ordenes.de.reparacion'].new({
