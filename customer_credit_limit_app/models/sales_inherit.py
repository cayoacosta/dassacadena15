...
class sale_order(models.Model):
    ...
    def _compute_total_customer_limit(self):
        ...
        if self.currency_id and self.currency_id != self.company_id.currency_id:
            for total in sale_order_ids:
                sale_amount += total.currency_id._convert(total.amount_total, total.company_id.currency_id, self.company_id, total.confirmation_date or fields.Date.today())
        else:
            for total in sale_order_ids:
    ...
    def action_confirm(self):
        ...
        due_invoice = self.env['account.invoice'].search([('type','=','out_invoice'),('date_due','<',date_today),('state','=','open'),('partner_id','=',self.partner_id.id)])
        
        if self.currency_id and self.currency_id != self.company_id.currency_id:
            amount = self.currency_id._convert(self.amount_total, self.company_id.currency_id, self.company_id, self.confirmation_date or fields.Date.today())
