...
class AccountInvoiceLine(models.Model):
    ...
    # Compute Section
    @api.depends(
        'purchase_price', 'quantity', 'price_subtotal') #, 'move_id.type')
    def _compute_multi_margin(self):
        for line in self.filtered(
                lambda l: l.product_id and
                l.move_id.type in ('out_invoice', 'out_refund')):
            ...
            if record.margin_percent:
                if record.purchase_price:
                    if record.product_id.taxes_id.amount:
                        if record.margin_percent > -9999.99 and record.margin_percent < 9999.99:
