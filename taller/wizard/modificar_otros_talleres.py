...
class ModificarOtrosTalleres(models.TransientModel):
    ...
    _description = 'modificar otros talleres wizard'
    ...
    def action_create(self):
        ...
        body ="Otros talleres <br/> Subtotal: -> %d" %self.reparacion_id.amount_untaxed_otros,  "<br/>" "Total: -> %d" %self.reparacion_id.amount_total_refacciones
    ...
    def _prepare_purchase_order_line(self, line, po):
        ...
        return {
            ...
            'price_unit': line.costo_proveedor_ot,
    ...
    def _prepare_purchase_order(self,  partner):
        ...
        return {
            ...
            'tipo':self.tipo,
            'journal_id':self.reparacion_id.journal_id.id,
...
class ModificarOtrosTalleresWizardLine(models.TransientModel):
    ...
    _description = 'modificar otros talleres wizard line'
    ...
    costo_proveedor = fields.Float('Costo proveedor', digits=dp.get_precision('Product Price'), default=0.0)
    costo_proveedor_ot = fields.Float('Costo poveedor sin IVA', digits=dp.get_precision('Product Price'), default=0.0)
    tax_id = fields.Many2many('account.tax','modificar_otros_rel', 'modificar_tax', 'tax_id', string='Impuestos', domain=['|', ('active', '=', False), ('active', '=', True)])
    ...
    @api.onchange('pct_percent', 'costo_proveedor', 'costo_proveedor_ot')
    def onchange_pct_costo(self):
        if self.pct_percent and self.costo_proveedor:
            ...
            self.costo_proveedor_ot = self.costo_proveedor /(1+self.product_id.supplier_taxes_id.amount/100)
